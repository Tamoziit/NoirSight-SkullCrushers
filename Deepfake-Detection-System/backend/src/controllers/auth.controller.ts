...
export const signup = async (req: Request, res: Response) => {
	try {
		...
		if (newUser) {
			...
			await client.set(`NS-user:${newUser._id}`, JSON.stringify(payload));
			await client.expire(`NS-user:${newUser._id}`, 30 * 24 * 60 * 60);
...
export const login = async (req: Request, res: Response) => {
	try {
		...
		await client.set(`NS-user:${user._id}`, JSON.stringify(payload));
		await client.expire(`NS-user:${user._id}`, 30 * 24 * 60 * 60);
...
export const logout = async (req: Request, res: Response) => {
	try {
		...
		res.cookie("NS-jwt", "", { maxAge: 0 });
		await client.del(`NS-user:${userId}`);
